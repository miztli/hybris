...
- (void)loadAllProducts {
    ...
    [self.b2bService getCategoriesAndExecute:^(NSArray *foundCategories, NSError *error) {
...
- (void)addProductToCartFromCell:(HYBListViewCell *)cell selectedIndex:(NSInteger)selectedIndex {
    ...
    if (amount) {
        ...
        DDLogDebug(@"Adding product %@ in amount %d to cart ", product.firstVariantCode, amount.intValue);
        
        @weakify(self);
        [self.b2bService addProductToCurrentCart:product.firstVariantCode
                                          amount:amount
...
- (void)loadProducts {
    [self.b2bService getProductsAndExecute:^(NSArray *products, NSError *error) {
        if (error) {
            DDLogError(@"Problems during the retrieval of the products from the web service: %@", [error localizedDescription]);
        } else {
            [self processNewProducts:products];
...
- (void)doPerformSearch {
    ...
    @weakify(self);
    [self.b2bService getProductsByQuery:self.currentSearchQuery
